...
use anthropic_agent_sdk::types::{
    ClaudeAgentOptions, HookDecision, HookEvent, HookOutput, Message,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let matcher = anthropic_agent_sdk::hooks::HookMatcherBuilder::new(Some("*"))
    ...
    client
        .send_message("What files are in the current directory?")
        .await?;
    ...
    let blocking_hook = HookManager::callback(|_event_data, tool_name, _context| async move {
        ...
        if tool == "Write" || tool == "Edit" {
            ...
            return Ok(HookOutput {
                ...
                system_message: Some(format!("Tool {tool} was blocked by security policy")),
                hook_specific_output: None,
            });
        }
    ...
    let matcher = anthropic_agent_sdk::hooks::HookMatcherBuilder::new(None::<String>) // Match all
    ...
    client
        .send_message("Create a file called test.txt with 'hello world'")
        .await?;
    ...
    let matcher = anthropic_agent_sdk::hooks::HookMatcherBuilder::new(Some("Bash"))
        .add_hook(bash_hook)
        .build();
    ...
    client
        .send_message("What is the current date and time?")
        .await?;
