...
async fn test_multiple_successive_messages() {
    ...
    while let Some(_msg) = timeout(Duration::from_secs(10), client.next_message())
        .await
        .ok()
        .flatten()
    {
...
async fn test_hook_receiver() {
    ...
    client.send_message("Can you read a file?").await.unwrap();
...
async fn test_message_flow_order() {
    ...
    while let Some(msg) = timeout(Duration::from_secs(10), client.next_message())
        .await
        .ok()
        .flatten()
    {
