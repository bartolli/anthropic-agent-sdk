...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let permission_callback =
        PermissionManager::callback(|tool_name, _tool_input, _context| async move {
    ...
    let sanitizing_callback =
        PermissionManager::callback(|tool_name, tool_input, _context| async move {
            let tool = tool_name.as_str();
    ...
    client
        .send_message("List all .rs files in the src directory")
        .await?;
    ...
    let context_aware_callback =
        PermissionManager::callback(|tool_name, _tool_input, context| async move {
            let tool = tool_name.as_str();
            println!("  [PERMISSION] Tool: {tool}");
            println!(
                "  [PERMISSION] Context suggestions: {:?}",
                context.suggestions
            );
    ...
    client
        .send_message("Find all TODO comments in the src directory")
        .await?;
